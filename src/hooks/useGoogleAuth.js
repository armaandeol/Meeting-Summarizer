...
export function useGoogleAuth() {
  ...
  const loginWithGoogle = useCallback(async () => {
    if (!gapiInitialized || !gisInitialized) {
      ...
      setAuthError("Google API not initialized yet");
      return;
    }
    ...
    try {
      console.log(
        "Attempting Google login with client ID:",
        CLIENT_ID?.substring(0, 5) + "..."
      );
      ...
      const client = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: (tokenResponse) => {
          if (tokenResponse && tokenResponse.access_token) {
            ...
            setAuthError("No access token received from Google");
          }
        },
      ...
      setAuthError(`Login failed: ${error.message || JSON.stringify(error)}`);
    }
  }, [gapiInitialized, gisInitialized]);
