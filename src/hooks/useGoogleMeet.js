...
export function useGoogleMeet(isGoogleAuthenticated) {
  ...
  const createMeetingLink = useCallback(async (title = 'Meeting Summarizer Meeting', startTime = new Date(), duration = 60) => {
    ...
    try {
      console.log('Creating Google Meet meeting:', { title, startTime, duration });
      ...
      setError(error.result?.error?.message || error.message || 'Failed to create meeting');
      return null;
    } finally {
